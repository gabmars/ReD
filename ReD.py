...
OUTPUT_DIR = 'output/'
# Style image to use.
STYLE_IMAGE = 'style/style7.jpg'
# Content image to use.
CONTENT_IMAGE = 'content/willy_wonka_old.jpg'
# Image dimensions constants.
IMAGE_WIDTH = 800
IMAGE_HEIGHT = 600
COLOR_CHANNELS = 3
sim=Image.open(STYLE_IMAGE)
cim=Image.open(CONTENT_IMAGE)
cim=cim.resize((IMAGE_WIDTH,IMAGE_HEIGHT),Image.ANTIALIAS)
sim=sim.resize((IMAGE_WIDTH,IMAGE_HEIGHT),Image.ANTIALIAS)
cim.save(CONTENT_IMAGE)
sim.save(STYLE_IMAGE)
#%%
# Algorithm constants
...
def style_loss_func(sess, model):
    ...
    def _gram_matrix(F, N, M):
        ...
        """
        Ft = tf.reshape(F, (M, N))
        r = tf.matmul(tf.transpose(Ft), Ft)
        return r
...
for it in range(ITERATIONS):
    ...
    s1 = sess.run(tf.reduce_sum(mixed_image))
    s2 = sess.run(total_loss)
